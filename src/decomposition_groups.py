...
methods = ['kmeans', 'DBSCAN', 'HDBSCAN', 'OPTICS', 'DPC', 'DPC_modified', 'DPC_remodified']
...
def radar(year, method, group): # the 'group' here is the name of a particular group
    ...
    for i in range(len(values)):
        ax.plot(angles, values[i], color=cmap_50(i / 49), linewidth=2.5, label=states_within_group[i], marker='o')
    # set labels
    ...
    ax.set_title(f'{group}', fontweight='bold', fontfamily='Times New Roman', pad=70, fontsize=36)
    plt.savefig(f'../outputs/figures/decompose_group/{method}_{year}_{group}.jpg', bbox_inches='tight', dpi=500)
    return
